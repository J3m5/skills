...
Ask the user for the subjective fields first, then run the helper script from this skill directory with those answers:

```bash
python3 scripts/collect_local_setup.py \
  --workspace "$PWD" \
  --format markdown \
